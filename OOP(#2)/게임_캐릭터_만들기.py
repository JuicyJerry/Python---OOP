...
# 대위가 맡은 부분은 게임 캐릭터를 ‘클래스’로 작성하는 것입니다. 이미 객체의 속성과 행동을 뭐로 할지에 대해서는 생각을 마친 상태입니다.
...
class GameCharacter:
    ...
    def __init__(self, name, hp, power):
        ...
        self.name = str(name) # string, name
        self.hp = int(hp)    # int, hit point
        self.power = int(power) # int, power
    ...
    def get_attacked(self, damage):
        ...
        if self.is_alive() == True: # 괄호 안에 self를 넣으면 어떻게 되는거지?
            return damage
        elif self.damage == False:
            return "{}dms 이미 죽었습니다.".format(self.name)
        elif self.power > self.hp:
            return self.hp == 0
    ...
    def __str__(self):  # self, name, hp, power라고 하면 왜 에러가 나오는거지
        ...
        return "{}님의 {}는 {}만큼 남았습니다.".format(self.name, self.hp, self.power)
