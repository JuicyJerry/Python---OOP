...
class Clock:
    ...
    def __init__(self, hour, minute, second):
        ...
        """
        Clock.HOURS = Counter(24).limit
        Clock.MINUTES = Counter(60).limit
        Clock.SECONDS = Counter(60).limit
    ...
    def set(self, hour, minute, second):
        ...
        if 0 <= hour < Clock.HOURS:
            self.hour = hour
        else:
            self.hour = 0

        if 0 <= minute < Clock.MINUTES:
            self.minute = minute
        else:
            self.minute = 0

        if 0 <= second < Clock.SECONDS:
            self.second = second
        else:
            self.second = 0
    ...
    def tick(self):
        ...
        elif self.minute == Clock.MINUTES:
            self.minute = 0
            self.hour += 1

        elif self.hour == Clock.HOURS:
            self.hour = 0
